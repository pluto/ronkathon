...
use crate::{
  extension::BinomiallyExtendable, field::Field, field_to_array, AbstractExtensionField,
  AbstractField, ExtensionField, Packable, TwoAdicField,
};
...
impl<AF: AbstractField, const D: usize> Default for BinomialExtensionField<AF, D> {
  fn default() -> Self { Self { value: array::from_fn(|_| AF::zero()) } }
}

impl<AF: AbstractField, const D: usize> From<AF> for BinomialExtensionField<AF, D> {
  fn from(x: AF) -> Self { Self { value: field_to_array::<AF, D>(x) } }
}
...
impl<F: BinomiallyExtendable<D>, const D: usize> HasFrobenius<F> for BinomialExtensionField<F, D> {
  ...
  fn frobenius(&self) -> Self { self.repeated_frobenius(1) }
  ...
  AF::F: BinomiallyExtendable<D>,
{
  ...
  fn zero() -> Self { Self { value: field_to_array::<AF, D>(AF::zero()) } }

  fn one() -> Self { Self { value: field_to_array::<AF, D>(AF::one()) } }

  fn two() -> Self { Self { value: field_to_array::<AF, D>(AF::two()) } }

  fn neg_one() -> Self { Self { value: field_to_array::<AF, D>(AF::neg_one()) } }

  fn from_f(f: Self::F) -> Self { Self { value: f.value.map(AF::from_f) } }

  fn from_bool(b: bool) -> Self { AF::from_bool(b).into() }

  fn from_canonical_u8(n: u8) -> Self { AF::from_canonical_u8(n).into() }

  fn from_canonical_u16(n: u16) -> Self { AF::from_canonical_u16(n).into() }

  fn from_canonical_u32(n: u32) -> Self { AF::from_canonical_u32(n).into() }

  /// Convert from `u64`. Undefined behavior if the input is outside the canonical range.
  fn from_canonical_u64(n: u64) -> Self { AF::from_canonical_u64(n).into() }

  /// Convert from `usize`. Undefined behavior if the input is outside the canonical range.
  fn from_canonical_usize(n: usize) -> Self { AF::from_canonical_usize(n).into() }

  fn from_wrapped_u32(n: u32) -> Self { AF::from_wrapped_u32(n).into() }

  fn from_wrapped_u64(n: u64) -> Self { AF::from_wrapped_u64(n).into() }

  fn generator() -> Self { Self { value: AF::F::ext_generator().map(AF::from_f) } }
...
impl<F: BinomiallyExtendable<D>, const D: usize> Field for BinomialExtensionField<F, D> {
  ...
  fn order() -> BigUint { F::order().pow(D as u32) }
}
  ...
  AF::F: BinomiallyExtendable<D>,
{
  ...
  #[inline]
  fn neg(self) -> Self { Self { value: self.value.map(AF::neg) } }
}
...
where
  ...
  AF::F: BinomiallyExtendable<D>,
{
  fn add_assign(&mut self, rhs: AF) { *self = self.clone() + rhs; }
}
...
{
  #[inline]
  fn sub_assign(&mut self, rhs: AF) { *self = self.clone() - rhs; }
}
  ...
  AF::F: BinomiallyExtendable<D>,
{
  ...
  #[inline]
  fn mul(self, rhs: AF) -> Self { Self { value: self.value.map(|x| x * rhs.clone()) } }
}
...
where
  ...
  AF::F: BinomiallyExtendable<D>,
{
  fn mul_assign(&mut self, rhs: AF) { *self = self.clone() * rhs; }
}
  ...
  AF::F: BinomiallyExtendable<D>,
{
  ...
  fn from_base(b: AF) -> Self { Self { value: field_to_array(b) } }
  ...
  #[inline]
  fn from_base_fn<F: FnMut(usize) -> AF>(f: F) -> Self { Self { value: array::from_fn(f) } }

  fn as_base_slice(&self) -> &[AF] { &self.value }
}
  ...
  for BinomialExtensionField<F, D>
{
  ...
  fn two_adic_generator(bits: usize) -> Self { Self { value: F::ext_two_adic_generator(bits) } }
}
...
fn cubic_mul<AF: AbstractField>(a: &[AF], b: &[AF], w: AF::F) -> [AF; 3] {
  ...
  let c0 = a0_b0.clone()
    + ((a[1].clone() + a[2].clone()) * (b[1].clone() + b[2].clone())
      ...
      * AF::from_f(w);
  let c1 =
    (a[0].clone() + a[1].clone()) * (b[0].clone() + b[1].clone()) - a0_b0.clone() - a1_b1.clone()
