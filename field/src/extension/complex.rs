...
impl<F: ComplexExtendable> BinomiallyExtendable<2> for F {
  ...
  fn ext_generator() -> [Self; 2] { F::complex_generator().value }
}

/// Convenience methods for complex extensions
impl<AF: AbstractField> Complex<AF> {
  pub const fn new(real: AF, imag: AF) -> Self { Self { value: [real, imag] } }

  pub fn new_real(real: AF) -> Self { Self::new(real, AF::zero()) }

  pub fn new_imag(imag: AF) -> Self { Self::new(AF::zero(), imag) }

  pub fn real(&self) -> AF { self.value[0].clone() }

  pub fn imag(&self) -> AF { self.value[1].clone() }

  pub fn conjugate(&self) -> Self { Self::new(self.real(), self.imag().neg()) }

  pub fn norm(&self) -> AF { self.real().square() + self.imag().square() }

  pub fn to_array(&self) -> [AF; 2] { self.value.clone() }
...
{
  fn w() -> Self { <F as HasComplexBinomialExtension<D>>::w() }

  fn dth_root() -> Self { <F as HasComplexBinomialExtension<D>>::dth_root() }

  fn ext_generator() -> [Self; D] { <F as HasComplexBinomialExtension<D>>::ext_generator() }
}
...
where F: HasTwoAdicComplexBinomialExtension<D>
{
  ...
  fn ext_two_adic_generator(bits: usize) -> [Self; D] { F::complex_ext_two_adic_generator(bits) }
}
