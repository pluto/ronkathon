...
use crate::{
  exponentiation::exp_u64_by_squaring,
  packed::{PackedField, PackedValue},
  Packable,
};
...
pub trait AbstractField:
  ...
  #[must_use]
  fn cube(&self) -> Self { self.square() * self.clone() }
  ...
  #[must_use]
  #[inline]
  fn exp_u64(&self, power: u64) -> Self { Self::F::exp_u64_generic(self.clone(), power) }
  ...
  #[must_use]
  fn powers(&self) -> Powers<Self> { self.shifted_powers(Self::one()) }
...
pub trait Field:
  ...
  #[must_use]
  #[inline]
  fn mul_2exp_u64(&self, exp: u64) -> Self { *self * Self::two().exp_u64(exp) }

  /// self / 2^exp
  #[must_use]
  #[inline]
  fn div_2exp_u64(&self, exp: u64) -> Self { *self / Self::two().exp_u64(exp) }
  ...
  #[must_use]
  fn inverse(&self) -> Self { self.try_inverse().expect("Tried to invert zero") }
...
pub trait ExtensionField<Base: Field>: Field + AbstractExtensionField<Base> {
  ...
  fn is_in_basefield(&self) -> bool { self.as_base_slice()[1..].iter().all(Field::is_zero) }
...
impl<AF: AbstractField> AbstractExtensionField<AF> for AF {
  ...
  fn from_base_fn<F: FnMut(usize) -> AF>(mut f: F) -> Self { f(0) }

  fn as_base_slice(&self) -> &[AF] { slice::from_ref(self) }
}
