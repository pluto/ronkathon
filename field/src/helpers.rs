...
use core::{array, iter::Sum, ops::Mul};

use num_bigint::BigUint;

use crate::{field::Field, AbstractField, PrimeField, PrimeField32, TwoAdicField};
...
pub fn scale_vec<F: Field>(s: F, vec: Vec<F>) -> Vec<F> { vec.into_iter().map(|x| s * x).collect() }
