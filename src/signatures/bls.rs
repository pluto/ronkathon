...
use crate::{
  algebra::{
    field::{prime::PlutoScalarField, Field},
    group::FiniteCyclicGroup,
    Finite,
  },
  curve,
  curve::{pairing::pairing, pluto_curve::PlutoExtendedCurve, AffinePoint},
...
///
/// This simplistic implementation is for educational purposes and is not optimized for production
/// use.
fn hash_to_curve(msg: &[u8]) -> Result<AffinePoint<PlutoExtendedCurve>, BlsError> {
  ...
  loop {
    ...
    let candidate_x =
      <curve::pluto_curve::PlutoExtendedCurve as curve::EllipticCurve>::BaseField::from(x_val);
    // Compute y² = x³ + 3 using the curve equation: y² = x³ + 3.
    let x3 = candidate_x * candidate_x * candidate_x;
    let y2 =
      x3 + <curve::pluto_curve::PlutoExtendedCurve as curve::EllipticCurve>::BaseField::from(3u64);
