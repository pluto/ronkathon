...
//!
//! This implementation provides both interactive and non-interactive versions of the sumcheck protocol.
//!
...
//! - `compute_univariate_polynomial`: A helper method to compute the univariate polynomial for each round.
//!
...
//! ## Usage
//!
//! To use this implementation, create a `MultivariatePolynomial`, then use the `non_interactive_sumcheck_prove`
//! function to generate a proof, and `non_interactive_sumcheck_verify` to verify it.
//!
...
use crate::{algebra::field::FiniteField, polynomial::multivariate_polynomial::MultivariatePolynomial, random::{Random, RandomOracle}};
...
impl<F: FiniteField + Display> MultivariatePolynomial<F> {
  ...
  /// 1. It computes the total sum of the polynomial over all boolean inputs, which is the
  ///    claimed sum that the prover wants to prove.
  /// 2. It generates the first univariate polynomial g_1(X1), which is the first step in
  ///    reducing the multivariate sumcheck to a series of univariate sumchecks.
  ///
  ...
  ///   2. The first univariate polynomial (MultivariatePolynomial<F>): g_1(X1), which is
  ///      actually univariate despite the type name.
  ...
  /// # Returns
  ///
  /// * `MultivariatePolynomial<F>` - The univariate polynomial g_i(X_i) for the i-th round.
  ///   Despite the type name, this polynomial is univariate in X_i.
  ///
  ...
  ///    polynomial in X_i.
  ///
  /// 2. Sum Consistency: The sum of g_i(X_i) over {0,1} equals g_{i-1}(r_{i-1}), where r_{i-1}
  ///    is the random challenge from the previous round.
  ///
  /// 3. Randomized Reduction: g_i(X_i) reduces the sum check for i variables to a sum check
  ...
  /// # Returns
  ///
  /// * `MultivariatePolynomial<F>` - The final univariate polynomial for the last round.
  ///   This polynomial is univariate in the last remaining variable.
  ///
  /// # Note
  ///
  /// This function relies on `compute_univariate_polynomial` to perform the actual computation
...
///
/// 1. `claimed_sum`: The total sum claimed by the prover. This is the value that the
///    verifier wants to check without computing the entire sum themselves.
///
/// 2. `univariate_poly`: The first univariate polynomial g_1(X_1) provided by the prover.
///    This polynomial is supposed to represent the sum over all but the first variable.
///
/// The verification process involves:
///
/// 1. Checking that the provided polynomial is indeed univariate. This ensures that
///    the prover is following the protocol correctly by reducing one variable at a time.
///
/// 2. Verifying that g_1(0) + g_1(1) equals the claimed sum. This check is fundamental
///    because it connects the univariate polynomial to the original multivariate sum.
///    If this equality holds, it suggests that the prover has correctly computed the
///    univariate polynomial for the first round.
///
/// 3. Generating a random challenge. This challenge will be used in subsequent rounds
///    and is crucial for the security of the protocol. It ensures that the prover
///    cannot predict or manipulate future rounds.
///
/// # Arguments
///
...
///
/// This function is crucial for verifying the correctness of each intermediate step in the sumcheck protocol.
/// It ensures that the prover is following the protocol correctly and not deviating from the expected behavior.
///
/// # Arguments
///
/// * `round`: The current round number of the sumcheck protocol. This is needed to keep track of which variable
///            is being eliminated in the current round.
/// * `challenge`: The random challenge from the previous round. This is used to evaluate the previous round's
///                polynomial and connect it to the current round.
/// * `previous_univariate_poly`: The univariate polynomial from the previous round. This is needed to verify
///                               the consistency between rounds.
/// * `current_univariate_poly`: The univariate polynomial for the current round. This is the polynomial that
///                              the prover claims represents the sum over the current variable.
///
/// # Returns
///
/// A tuple containing:
...
///
/// 1. `round`: Keeps track of the protocol's progress and ensures variables are eliminated in order.
/// 2. `challenge`: Connects the current round to the previous one, preventing the prover from deviating.
...
/// - The univariate nature of the current polynomial (ensuring one variable is eliminated per round).
/// - The consistency between rounds (g_i(r_{i-1}) = g_{i-1}(0) + g_{i-1}(1)).
/// - Generate a new random challenge for the next round, maintaining the protocol's unpredictability.
...
/// # Type Parameters
///
/// * `F`: A type that implements the `FiniteField` trait, representing the field over which
///        the sumcheck protocol is performed.
...
/// # Type Parameters
///
/// * `F` - A finite field type that implements necessary traits for arithmetic,
///         random number generation, conversion to bytes, and display.
...
///
/// 1. Check the consistency of the first round's claimed sum with the provided univariate polynomial.
/// 2. Verify the consistency between consecutive rounds' univariate polynomials.
/// 3. Confirm that the final evaluation matches the original multivariate polynomial at the challenge point.
///
/// The non-interactive nature of this proof system comes from the use of a random oracle to generate
/// challenges, which both the prover and verifier can compute independently.
///
/// # Arguments
///
