...
//!
//! This implementation uses a novel and highly efficient representation for multivariate
//! polynomials. Each term in the polynomial is represented as a key-value pair in a HashMap, where:
...
//! 2. Fast Term Lookup: The use of BTreeMap for exponents allows for quick term identification and
//!    manipulation.
//! 3. Ordered Operations: BTreeMap's ordered nature facilitates efficient polynomial arithmetic.
//! 4. Memory Optimization: By using indices instead of full variable objects, we reduce memory
//!    usage.
//! 5. Flexible Degree Handling: This structure naturally accommodates polynomials of arbitrary
//!    degree.
//! 6. Efficient Iteration: Easy to iterate over terms, useful for various algorithms and
//!    transformations.
//!
...
/// - A `BTreeMap<usize, usize>` as the key, mapping variable indices to their exponents. This
///   allows for efficient storage and manipulation of sparse polynomials.
...
impl<F: FiniteField> MultivariatePolynomial<F> {
  ...
  ///
  /// This function creates an empty `MultivariatePolynomial`, which is equivalent to the zero
  /// polynomial. The zero polynomial has no terms, and evaluates to zero for all inputs.
  pub fn new() -> Self { Self { terms: HashMap::new() } }
  ...
  /// ```
  /// use your_crate::{
  ///   MultivariatePolynomial, MultivariateTerm, MultivariateVariable, PlutoBaseField,
  /// };
  ///
  /// let poly = MultivariatePolynomial::<PlutoBaseField>::from_terms(vec![
  ///   MultivariateTerm::new(
  ///     vec![MultivariateVariable { index: 0, exponent: 2 }, MultivariateVariable {
  ///       index:    1,
  ///       exponent: 1,
  ///     }],
  ...
  ///   ),
  ///   MultivariateTerm::new(vec![], PlutoBaseField::new(1)),
  /// ]);
  ///
  ...
  /// # Arguments
  ///
  /// * `exponents` - A `BTreeMap` where the keys are variable indices and the values are their
  ///   exponents.
  ///
  /// # Returns
  ///
  /// * `Some(&F)` if a term with the given exponents exists in the polynomial, where `F` is the
  ///   coefficient.
  ...
  pub fn degree(&self) -> usize {
    self.terms.keys().map(|exponents| exponents.values().sum::<usize>()).max().unwrap_or(0)
  }
  ...
  fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
    ...
    for (exponents, coeff) in self.terms.iter().sorted_by(|(a_exp, _), (b_exp, _)| {
      a_exp
        ...
        .map_or(std::cmp::Ordering::Equal, |(..)| std::cmp::Ordering::Less)
    }) {
      if !first {
        write!(f, " + ")?;
      }
...
impl<F: FiniteField, const D: usize> From<Polynomial<Monomial, F, D>>
...
impl<F: FiniteField, const D: usize> Polynomial<Monomial, F, D> {
  ...
  /// # Arguments
  ///
  /// * `variable_index` - The index of the variable to use in the resulting multivariate
  ///   polynomial.
  ///
  /// # Returns
  ///
...
impl MultivariateVariable {
  ...
  pub fn new(index: usize, exponent: usize) -> Self { MultivariateVariable { index, exponent } }
}
