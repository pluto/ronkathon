...
fn setup() -> (Vec<AffinePoint<G1Curve>>, Vec<AffinePoint<G2Curve>>) {
  ...
  for i in 0..7 {
    ...
    let result = AffinePoint::<G1Curve>::generator() * t_power;
    srs_g1_points.push(result);
    // G2 Group

    // degree two divisor poly
    if i < 2 {
      let result = AffinePoint::<G2Curve>::generator() * t_power.into();
...
fn commit(coefs: Vec<GF101>, g1_srs: Vec<AffinePoint<G1Curve>>) -> AffinePoint<G1Curve> {
...
mod tests {
  ...
  fn test_commit() {
    ...
    assert_eq!(commit, AffinePoint::<G1Curve>::new(GF101::new(1), GF101::new(2)));
  }
}
