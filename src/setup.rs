...
fn setup(degree: u64) -> (Vec<AffinePoint<C101>>, Vec<AffinePoint<G2Curve>>) {
  ...
  let tau = BigUint::from_bytes_le(&tau_bytes);
  ...
  for i in 1..degree + 1 {
    // G1 Group
    let result = AffinePoint::<C101>::generator() * tau.pow(i as u32);
